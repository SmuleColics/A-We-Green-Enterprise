                        ...
                        <tbody>
                            @foreach ($tasks as $task)
                                <tr data-status="{{ $task->status }}" data-task-id="{{ $task->id }}">
                                    ...
                                    <td>
                                        <div>
                                            <p class="mb-0 fw-semibold">{{ $task->assessment->client->user->full_name ?? 'N/A' }}</p>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <span class="material-symbols-outlined fs-18 @if($task->is_overdue) text-danger @endif">event</span>
                                            <div>
                                                <p class="mb-0">{{ $task->due_date->format('M j, Y') }}</p>
                                                @if ($task->is_overdue && !$task->isCompleted())
                                                    <p class="mb-0 small text-danger fw-semibold">{{ $task->days_until_due }} days overdue</p>
                                                @elseif (!$task->isCompleted())
                                                    <p class="mb-0 small text-muted">{{ $task->days_until_due }} days remaining</p>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="badge {{ $task->status_badge }}">
                                            <span class="material-symbols-outlined" style="font-size: 14px;">{{ $task->status_icon }}</span>
                                            {{ $task->status }}
                                        </span>
                                    </td>
                                    <td>
