...
@endif
<form method="POST" action="{{ route('employee.assessments.form.update', $assessment) }}">
@csrf @method('PUT')
...
<div class="d-flex justify-content-end gap-2"><a target="_blank" href="{{ route('employee.assessments.form.print', $assessment) }}" class="btn btn-outline-secondary">Print / Preview PDF</a>@if($quotation)<button class="btn btn-success">Update Assessment Form</button>@else<button class="btn btn-success">Save, Create & Send Quotation</button>@endif</div>
</form></div>
@endsection
@section('scripts')
<script>
